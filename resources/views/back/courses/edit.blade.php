<form action="{{ route('back.courses.update', ['course' => $course]) }}" method="post" id="edit_form" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="row">
        ...
        @include('components.forms.select', [
            'name' => 'instructor_id',
            'label' => __('lang.instructor') ?? 'Instructor',
            'options' => $instructors->pluck('name', 'id')->toArray(),
            'value' => old('instructor_id', $course->instructor_id),
            'required' => true,
            'placeholder' => __('lang.select_instructor') ?? 'Select Instructor',
            'class' => 'col-12 col-md-6'
        ])
